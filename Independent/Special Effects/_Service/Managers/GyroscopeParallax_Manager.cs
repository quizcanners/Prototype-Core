        ...
        public class GyroscopeParallaxManager : IPEGI, IPEGI_ListInspect
        {
            ...
            public void ManagedLateUpdate()
            {
                ...
                if (!Application.isEditor)
                {
                    ...
                    AccumulatedOffset += new Vector2(rate.y, -rate.x) * _sensitivity;
            ...
            public void Inspect()
            {
                ...
                var on = Gyro.enabled;
                if (" Gyro.enabled".toggleIcon(ref on).nl())
                    Gyro.enabled = on;

                if (Application.isEditor)
                {
                    "Previous: {0}".F(previousRotation).nl();
                    "Emulate in Editor".toggleIcon(ref _emulateTiltsInEditor).nl();
                }

                "Offset: {0}".F(AccumulatedOffset).nl();

                "Sensitivity".edit(ref _sensitivity).nl();

                "Fade Speed".edit(ref _centralizeSpeed).nl();

                USE_PARALLAX.Nested_Inspect().nl();

                if (USE_PARALLAX.Enabled)
                    PARALLAX_OFFSET.ToString().write_ForCopy(showCopyButton: true).nl();
            }
            ...
            public void InspectInList(ref int edited, int index)
            {
                ...
                if ("Gyroscope".ClickLabel() || icon.Enter.Click())
                    edited = index;
            }
