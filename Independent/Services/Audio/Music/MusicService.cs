...
namespace QuizCanners.IsItGame
{
    ...
    public class MusicService : IsItGameServiceBase, INeedAttention
    {
        ...
        private Preference_Bool _musicIsOn = new Preference_Bool("WantMusic", defaultValue: true);
        private Preference_Float _musicVolume = new Preference_Float("MusicVolume", defaultValue: 0.5f);

        private readonly Dictionary<string, float> _perSongPlaybeckResume = new Dictionary<string, float>();
        ...
        private void Update()
        {
            ...
            if (TryEnterIfStateChanged())
            {
                ...
                if (GameStateMachine.TryChange(ref song)) 
                    Play(song);
            }
        ...
        public override void Inspect()
        {
            ...
            {
                if ("Song To Play".editEnum(ref _debugCoreMusic) || icon.Play.Click().nl())
                    _debugCoreMusic.Play();

                "Volume".edit(50, ref _targetVolumeScale, 0, 2).nl();

                if (!sourceA && !sourceB) 
                {
                    "No Audio Sources Assigned".writeWarning();

                    if ("Create Audio Sources".Click().nl()) 
                    {
                ...
                {
                    "Active Playing: {0} Clip: {1}".F(ActiveSource.isPlaying, ActiveSource.clip).nl();
                    "Fading Playing: {0} Clip: {1}".F(FadingSource.isPlaying, FadingSource.clip).nl();
                }
            }

            "Music".edit_enter_Inspect(ref Music, ref _inspectedStuff, 0).nl();
        }
    ...
    public static class CoreMusicExtension
    {
        public static void Play(this IigEnum_Music music, bool skipTransition = false) =>
            Service.Try<MusicService>(s => s.Play(music, skipTransition: skipTransition));
    }
